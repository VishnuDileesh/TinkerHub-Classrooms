...
| [05](classrooms/05-deploying-gemmaikuchat) | [Deploying GemmaikuChat — Render + Vercel](classrooms/05-deploying-gemmaikuchat) | Taking Module 4 from "runs on my laptop" to a real URL — the model on [Render](https://render.com) (why it can't go on Vercel), the backend on Render, the frontend on [Vercel](https://vercel.com), and the small, real code changes (env vars, CORS) that bridge local and deployed. |
| [06](classrooms/06-gemmaiku-inference-engine) | [The inference engine — Gemmaiku on Fly.io and Hugging Face](classrooms/06-gemmaiku-inference-engine) | Going past CPU hosting: what an inference engine actually does (batching, KV cache), deploying a GPU-backed Ollama machine on [Fly.io](https://fly.io) (bring-your-own-container), and a fully managed GPU endpoint on [Hugging Face Inference Endpoints](https://huggingface.co) — and an honest comparison of both. |
| [07](classrooms/07-gemmaikuchat-flutter) | [GemmaikuChat, in your pocket — a Flutter mobile client](classrooms/07-gemmaikuchat-flutter) | A second, independent client for Module 4's exact backend contract — a real Android/iOS app in [Flutter](https://flutter.dev), streaming tokens over the same SSE endpoint, with zero backend changes. Proof that a clean HTTP contract outlives any one frontend. |
